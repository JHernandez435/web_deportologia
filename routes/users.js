...
router.post('/user/new', (req, res) => {
    let { name, lastname, address, email, contact, status, password } = req.body;
...
router.get('/reset/:token', (req, res)=> {
    User.findOne({resetPasswordToken: req.params.token, resetPasswordExpires : {$gt : Date.now() } })
        .then(user => {
            ...
            res.render('./user/newpassword', {token : req.params.token});
        })
        .catch(err => {
            req.flash('error_msg', 'ERROR: '+err);
            res.redirect('/forgot');
...
router.get('/user/all', isAuthenticatedUser ,(req, res)=> {
    User.find({})
        .then(users => {
            res.render('./user/alluser', {users : users});
        })
        .catch(err => {
            req.flash('error_msg', 'ERROR: '+err);
...
router.get('/edituser/:id', isAuthenticatedUser,(req,res)=> {
